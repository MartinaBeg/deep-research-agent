#!/usr/bin/env python3
"""Deterministic grounding check: keep only facts whose quote LITERALLY appears in
the cited source. This is what makes the report impossible to hallucinate.
...
Prints the validated subset as JSON to stdout; stats go to stderr. No LLM is used.
"""
import os
import re
import sys
...
def main():
    ...
    for f in facts:
        ...
        if len(q) < 15:
            continue
        if q in source_text(f.get("url", "")):
            kept.append(f)

    # re-id sequentially so [F<id>] markers map cleanly
