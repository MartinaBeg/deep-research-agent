...
Usage:
    ...
    echo "https://a.com" | python scrape.py <out_dir>     # urls on stdin

Each page's markdown is saved to <out_dir>/<hash>.md (cached: existing files are
reused, so re-runs don't re-spend Firecrawl credits). Needs FIRECRAWL_API_KEY.
No LLM is used here.
"""
import os
import sys
...
def main():
    ...
    for u in urls:
        ...
        if os.path.exists(path) and os.path.getsize(path) > 0:
            ...
            continue
        try:
            r = fc.scrape(url=u, formats=["markdown"])
