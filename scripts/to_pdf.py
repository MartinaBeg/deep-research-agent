...
links, and a real page-numbered Table of Contents (two-pass layout). Pure-Python
(markdown2 + fpdf2), no system libraries, no LLM.
...
render_body(pdf)
pdf.output(OUT)
print(f"PDF written: {OUT}  ({len(content)} sections, {pdf.page_no()} pages)")
