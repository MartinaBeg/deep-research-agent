#!/usr/bin/env python3
"""Web search via Serper -> JSON list of result URLs (most-cited first).

Usage:
    python search.py "query one" "query two" ...
    echo "query" | python search.py        # queries on stdin, one per line

Needs SERPER_API_KEY in the environment. No LLM is used here.
"""
import os
import sys
import json
...
def main():
    ...
    urls = sorted(freq, key=lambda u: (-freq[u], len(u)))
